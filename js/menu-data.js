window.menuMeta = {
  "name": "Al Ponte di Schiavonia Trattoria e Pizzeria",
  "description": "\"La storia della trattoria 'Al Ponte di Schiavonia' ha inizio nel 1978, quando aprì le sue porte nel borgo di Schiavonia a Forlì. Il nome del ristorante trae origine dalla sua posizione adiacente ad un ponte che rappresentava un importante punto di passaggio per entrare in città. Questo luogo strategico, insieme alla bellezza del borgo, ha ispirato il nome della trattoria che è diventata nel corso degli anni un punto di riferimento per la comunità locale, offrendo piatti della cucina romagnola e non solo, preparati con materie prime d'eccellenza e serviti in un'atmosfera accogliente e cordiale.\"",
  "cover": "3.00",
  "telephone": "0543 29448",
  "source": "Dishcovery API v3"
};

window.menuData = {
  "antipasti": [
    {
      "id": 307352,
      "name": "Tartare di Controfiletto Irlandese con Senape al Miele e Frutta Fresca",
      "price": 12.0,
      "description": "Preparato con carne cruda di controfiletto di manzo irlandese, tagliata finemente a coltello e servita con cubetti di frutta fresca. La delicatezza della carne si fonde con la freschezza della frutta, creando un'esperienza gastronomica unica, accompagnata dalla nota saporita e delicata della senape al miele.",
      "ingredients": "",
      "allergens": [
        "Senape"
      ],
      "allergens_source": "Dishcovery",
      "image": "assets/menu-images/307352_Tartare_di_Controfiletto_Irlandese_con_Senape_al_Miele_e_Frutta_Fresca.webp",
      "payoff": "",
      "order": 0
    },
    {
      "id": 307633,
      "name": "Acciughe del Mar Cantabrico e Burrata",
      "price": 11.0,
      "description": "Le acciughe del Mar Cantabrico sono considerate tra le migliori al mondo per la loro qualità e sapore unico. In questo antipasto, la cremosità della Burrata si fonde con il sapore salato delle acciughe, creando un contrasto di sapori equilibrato e delizioso. Il pane croccante e il burro aggiungono morbidezza e completano l'esperienza gustativa.",
      "ingredients": "",
      "allergens": [
        "Latte e derivati",
        "Pesce"
      ],
      "allergens_source": "Dishcovery",
      "image": "assets/menu-images/307633_Acciughe_del_Mar_Cantabrico_e_Burrata.webp",
      "payoff": "",
      "order": 1
    },
    {
      "id": 307611,
      "name": "Bruciatini",
      "price": 8.0,
      "description": "I Bruciatini sono un piatto tipico della tradizione culinaria della Romagna, a base di radicchio fresco e pancetta cotta in padella con aceto balsamico. Questi ingredienti vengono abilmente combinati per creare un'esplosione di sapori e profumi unici della cucina romagnola. Il radicchio fresco, croccante e leggermente amarognolo, si sposa perfettamente con la dolcezza della pancetta cotta e il sapore acidulo dell'aceto balsamico, creando un equilibrio di sapori unico. Il nome \"Bruciatini\" deriva dal fatto che la pancetta viene cotta fino a diventare croccante e bruciata, aggiungendo una nota croccante al piatto.",
      "ingredients": "",
      "allergens": [],
      "allergens_source": "",
      "image": "",
      "payoff": "",
      "order": 2
    },
    {
      "id": 493656,
      "name": "Flan di Zucca e Porri su Crema di Gorgonzola",
      "price": 8.0,
      "description": "La polpa di zucca e i porri vengono combinati in una crema morbida e aromatica, che viene cotta in piccole porzioni. Il flan viene servito su una crema di Gorgonzola, che conferisce un sapore deciso e cremosità al piatto. Questa combinazione di sapori rende il piatto un'esplosione di sapori e consistenze, con la dolcezza della zucca e dei porri che si fondono perfettamente con il sapore intenso e piccante del formaggio.",
      "ingredients": "",
      "allergens": [
        "Latte e derivati"
      ],
      "allergens_source": "Dishcovery",
      "image": "assets/menu-images/493656_Flan_di_Zucca_e_Porri_su_Crema_di_Gorgonzola.webp",
      "payoff": "",
      "order": 3
    },
    {
      "id": 307614,
      "name": "Selezione Di Affettati Tipici Con Piadina Romagnola",
      "price": 10.0,
      "description": "Questo piatto consiste in una scelta di affettati locali come salame, pancetta, prosciutto e mortadella, accompagnati da una piadina romagnola calda e morbida. La piadina è un tipo di pane piatto originario della Romagna, a base di farina, acqua, sale e lievito, cotta su una teglia di terracotta detta \"testo\".",
      "ingredients": "Salame, Prosciutto di Parma, Pancetta, Mortadella, Piadina",
      "allergens": [
        "Glutine",
        "Latte e derivati"
      ],
      "allergens_source": "Dishcovery",
      "image": "",
      "payoff": "",
      "order": 4
    },
    {
      "id": 307616,
      "name": "Degustazione di Formaggi del Territorio e Piadina Romagnola",
      "price": 10.0,
      "description": "Un' esperienza gastronomica che permette di assaporare alcuni dei formaggi più pregiati e tipici della Romagna, accompagnati dalla classica piadina romagnola, il pane tipico della regione. Tra i formaggi degustati, ci sono il formaggio di Fossa di Sogliano DOP, un formaggio a pasta semi-dura dal sapore intenso e dal profumo caratteristico; lo Squacquerone DOP, un formaggio fresco e cremoso che si scioglie in bocca; il Scoparolo, un formaggio semiduro dal sapore deciso e piccante; e infine il Pecorino Romagnolo, un formaggio a base di latte di pecora dal sapore deciso e speziato. La piadina romagnola, invece, è il pane tipico della regione, preparato con farina, acqua, strutto e sale e cotto su una padella in ghisa. È un alimento versatile che si presta ad essere farcito con ogni tipo di ingrediente, ma si sposa perfettamente con i formaggi della degustazione.",
      "ingredients": "Pecorino, Scoparolo, Pecorino di Fossa di Sogliano, Squacquerone DOP",
      "allergens": [
        "Glutine",
        "Latte e derivati"
      ],
      "allergens_source": "Dishcovery",
      "image": "assets/menu-images/307616_Degustazione_di_Formaggi_del_Territorio_e_Piadina_Romagnola.webp",
      "payoff": "",
      "order": 5
    },
    {
      "id": 307627,
      "name": "Cozze alla Tarantina",
      "price": 12.0,
      "description": "Le cozze alla tarantina sono un piatto tradizionale della Puglia, preparato con le migliori cozze del Mediterraneo. Le cozze vengono aperte e cotte con una salsa leggermente piccante a base di pomodoro, aglio, prezzemolo e peperoncino. Questo piatto rappresenta un'esplosione di sapori, che si fondono perfettamente insieme per creare un'esperienza culinaria autentica",
      "ingredients": "Cozze, Pomodoro, Aglio, Prezzemolo",
      "allergens": [
        "Molluschi"
      ],
      "allergens_source": "Dishcovery",
      "image": "assets/menu-images/307627_Cozze_alla_Tarantina.webp",
      "payoff": "",
      "order": 6
    }
  ],
  "primi": [
    {
      "id": 307635,
      "name": "Tagliatelle al Ragù della Nonna Franca",
      "price": 10.0,
      "description": "Le Tagliatelle al Ragù della Nonna Franca sono un piatto profondamente radicato nella tradizione culinaria romagnola. Questo piatto è un omaggio ai sapori autentici e alla semplicità rustica della regione. Le tagliatelle, fatte in casa con amore e cura, sono il letto perfetto per un ragù lento e ricco, cucinato pazientemente per ore. Il ragù è un connubio di carne macinata di qualità, soffritto di verdure fresche e una salsa di pomodoro che si addensa e si arricchisce di sapore durante la lunga cottura. Servite calde, con una generosa spolverata di Parmigiano Reggiano, le Tagliatelle al Ragù della Nonna Franca sono un piatto che riscalda il cuore e celebra la convivialità della tavola romagnola",
      "ingredients": "Sedano, Cipolla, Pasta fresca all’uovo, Carne di manzo e maiale, Carota",
      "allergens": [
        "Glutine",
        "Sedano",
        "Uova e derivati"
      ],
      "allergens_source": "Dishcovery",
      "image": "assets/menu-images/307635_Tagliatelle_al_Rag_della_Nonna_Franca.webp",
      "payoff": "",
      "order": 0
    },
    {
      "id": 307643,
      "name": "Cappelletti al Crudo di Parma, Ciliegini e Pesto di Rucola",
      "price": 12.0,
      "description": "",
      "ingredients": "Prosciutto Crudo, Farina, Pasta fresca all’uovo, Grana, Carne bollito",
      "allergens": [
        "Glutine",
        "Latte e derivati",
        "Uova e derivati"
      ],
      "allergens_source": "Dishcovery",
      "image": "assets/menu-images/307643_Cappelletti_al_Crudo_di_Parma_Ciliegini_e_Pesto_di_Rucola.webp",
      "payoff": "",
      "order": 1
    },
    {
      "id": 304039,
      "name": "Cappelletti al Ragù della Nonna Franca",
      "price": 12.0,
      "description": "I cappelletti sono un piatto tipico della cucina dell'Emilia-Romagna, in particolare della Romagna. Sono una pasta ripiena a forma di cappello ripiena di carne o formaggio e serviti in brodo o con sugo. Spesso vengono confusi con i tortellini, ma hanno una forma leggermente diversa e sono tipici di un'altra regione, ovvero l'Emilia.",
      "ingredients": "Grana Padano, Ricotta, Uova, Farina di tipo 1, maiale, Sale, pollo, Pepe",
      "allergens": [
        "Glutine",
        "Latte e derivati",
        "Sedano",
        "Uova e derivati"
      ],
      "allergens_source": "Dishcovery",
      "image": "assets/menu-images/304039_Cappelletti_al_Rag_della_Nonna_Franca.webp",
      "payoff": "",
      "order": 2
    },
    {
      "id": 307651,
      "name": "Tortelli di Ricotta e Spinaci  con Guanciale, Salvia, Crema di Burro Chiarificato",
      "price": 12.0,
      "description": "I tortelli di ricotta e spinaci sono un piatto tradizionale della cucina romagnola, preparati con sfoglia all'uovo e ripieno di ricotta e spinaci freschi. La loro consistenza morbida e vellutata si sposa perfettamente con il guanciale croccante, la salvia aromatica e la crema di burro chiarificato, creando un perfetto equilibrio di sapori e consistenze. Questo piatto rappresenta una vera delizia per il palato, che esalta la tradizione culinaria della regione.",
      "ingredients": "Guanciale, Pasta fresca all’uovo, Farina, Burro, Salvia",
      "allergens": [
        "Glutine",
        "Latte e derivati",
        "Uova e derivati"
      ],
      "allergens_source": "Dishcovery",
      "image": "assets/menu-images/307651_Tortelli_di_Ricotta_e_Spinaci_con_Guanciale_Salvia_Crema_di_Burro_Chiarificato.webp",
      "payoff": "",
      "order": 3
    },
    {
      "id": 307660,
      "name": "Strozzapreti ai Porcini con Scaglie di Pecorino di Fossa  di Sogliano",
      "price": 14.0,
      "description": "",
      "ingredients": "Porcini, Pecorino di Fossa di Sogliano, Farina di tipo 1",
      "allergens": [
        "Glutine"
      ],
      "allergens_source": "Dishcovery",
      "image": "assets/menu-images/307660_Strozzapreti_ai_Porcini_con_Scaglie_di_Pecorino_di_Fossa_di_Sogliano.webp",
      "payoff": "",
      "order": 4
    },
    {
      "id": 307678,
      "name": "Gnocchetti alle Vongole su Crema di Zucchine e Datterini",
      "price": 14.0,
      "description": "Gnocchetti di patate fatti in casa, arricchiti con vongole fresche e serviti su una cremosa base di zucchine e pomodori datterini. La consistenza morbida degli gnocchetti si sposa perfettamente con il sapore intenso e delicato delle vongole, mentre la crema di zucchine e pomodori datterini aggiunge un tocco di dolcezza e freschezza al piatto. Questo piatto è perfetto per gli amanti del mare e della cucina mediterranea, che vogliono gustare una pietanza ricca di sapori e di profumi.",
      "ingredients": "Vongole, Zucchine, Aglio, Pasta fresca fresca all’uovo, Pomodorini Ciliegino",
      "allergens": [
        "Cereali",
        "Molluschi"
      ],
      "allergens_source": "Dishcovery",
      "image": "assets/menu-images/307678_Gnocchetti_alle_Vongole_su_Crema_di_Zucchine_e_Datterini.webp",
      "payoff": "",
      "order": 5
    },
    {
      "id": 307669,
      "name": "Tagliolini allo Scoglio",
      "price": 17.0,
      "description": "",
      "ingredients": "Seppia, Gamberi, Polpo, Pasta fresca all’uovo, Cozze, Vongole, Aglio, Pomodoro, Prezzemolo",
      "allergens": [
        "Crostacei",
        "Glutine",
        "Molluschi",
        "Pesce",
        "Sedano",
        "Uova e derivati"
      ],
      "allergens_source": "Dishcovery",
      "image": "",
      "payoff": "",
      "order": 6
    },
    {
      "id": 520062,
      "name": "Lasagne della Tradizione Romagnola",
      "price": 12.0,
      "description": "Un piatto tipico della cucina romagnola, le lasagne sono preparate con sfoglia all'uovo, ragù di carne, besciamella e parmigiano reggiano, per un piatto saporito e sostanzioso.",
      "ingredients": "",
      "allergens": [
        "Latte e derivati",
        "Sedano",
        "Uova e derivati"
      ],
      "allergens_source": "Dishcovery",
      "image": "",
      "payoff": "",
      "order": 8
    }
  ],
  "pizze": [
    {
      "id": 304069,
      "name": "Americana",
      "price": 10.0,
      "description": "",
      "ingredients": "Fior di Latte, Pomodoro, Wurstel, Patate Fritte",
      "allergens": [
        "Glutine",
        "Latte e derivati"
      ],
      "allergens_source": "Dishcovery",
      "image": "",
      "payoff": "",
      "order": 0
    },
    {
      "id": 340814,
      "name": "Aromatica",
      "price": 13.0,
      "description": "",
      "ingredients": "Mozzarella Fiordilatte, Burrata, Prosciutto Crudo di Parma",
      "allergens": [],
      "allergens_source": "",
      "image": "",
      "payoff": "",
      "order": 1
    },
    {
      "id": 309811,
      "name": "Biancaneve",
      "price": 7.0,
      "description": "",
      "ingredients": "Fior di Latte",
      "allergens": [
        "Glutine",
        "Latte e derivati"
      ],
      "allergens_source": "Dishcovery",
      "image": "",
      "payoff": "",
      "order": 2
    },
    {
      "id": 307896,
      "name": "Biscara",
      "price": 11.0,
      "description": "",
      "ingredients": "Mozzarella di Bufala Dop, Pomodoro, Funghi Porcini, Salsiccia",
      "allergens": [
        "Glutine",
        "Latte e derivati"
      ],
      "allergens_source": "Dishcovery",
      "image": "",
      "payoff": "",
      "order": 3
    },
    {
      "id": 304071,
      "name": "Bomba",
      "price": 10.0,
      "description": "",
      "ingredients": "Fior di Latte, Pomodoro, Salame Piccante, Fagioli, Salsiccia, Cipolla",
      "allergens": [],
      "allergens_source": "",
      "image": "",
      "payoff": "",
      "order": 4
    },
    {
      "id": 304073,
      "name": "Boscaiola",
      "price": 10.5,
      "description": "",
      "ingredients": "Fior di Latte, Pomodoro, Funghi Porcini, Pancetta, Funghi Champignon",
      "allergens": [
        "Glutine",
        "Latte e derivati"
      ],
      "allergens_source": "Dishcovery",
      "image": "",
      "payoff": "",
      "order": 5
    },
    {
      "id": 309795,
      "name": "Capricciosa",
      "price": 12.0,
      "description": "",
      "ingredients": "Pomodoro, Prosciutto Cotto, Fior di Latte, Olive Taggiasche, Carciofi, Salsiccia, Funghi Champignon",
      "allergens": [
        "Glutine",
        "Latte e derivati"
      ],
      "allergens_source": "Dishcovery",
      "image": "",
      "payoff": "",
      "order": 7
    },
    {
      "id": 340813,
      "name": "Delicius",
      "price": 14.0,
      "description": "",
      "ingredients": "Mozzarella Fiordilatte, Burrata, Acciughe del Cantabrico, Olive Taggiasche",
      "allergens": [
        "Glutine",
        "Latte e derivati",
        "Pesce"
      ],
      "allergens_source": "Dishcovery",
      "image": "",
      "payoff": "",
      "order": 8
    },
    {
      "id": 309765,
      "name": "Diavola",
      "price": 9.0,
      "description": "",
      "ingredients": "Capperi, Fior di Latte, Pomodoro, Salame Piccante, Acciughe",
      "allergens": [
        "Glutine",
        "Latte e derivati",
        "Pesce"
      ],
      "allergens_source": "Dishcovery",
      "image": "",
      "payoff": "",
      "order": 9
    },
    {
      "id": 307897,
      "name": "Fuoco del Vesuvio",
      "price": 11.0,
      "description": "",
      "ingredients": "Fior di Latte, Pomodoro, Peperoni, Rucola, Salame Piccante, Olive Taggiasche",
      "allergens": [
        "Glutine",
        "Latte e derivati"
      ],
      "allergens_source": "Dishcovery",
      "image": "",
      "payoff": "",
      "order": 10
    },
    {
      "id": 307898,
      "name": "Ghiotta",
      "price": 9.5,
      "description": "",
      "ingredients": "Fior di Latte, Pomodoro, Scamorza, Salame Piccante, Cipolla",
      "allergens": [
        "Glutine",
        "Latte e derivati"
      ],
      "allergens_source": "Dishcovery",
      "image": "",
      "payoff": "",
      "order": 11
    },
    {
      "id": 307899,
      "name": "Mandolino",
      "price": 10.0,
      "description": "",
      "ingredients": "Mozzarella di Bufala Dop, Pomodoro, Olive Taggiasche, Origano, Pomini",
      "allergens": [
        "Glutine",
        "Latte e derivati"
      ],
      "allergens_source": "Dishcovery",
      "image": "",
      "payoff": "",
      "order": 13
    },
    {
      "id": 307540,
      "name": "Margherita",
      "price": 7.0,
      "description": "",
      "ingredients": "Pomodoro, Fior di Latte",
      "allergens": [
        "Glutine",
        "Latte e derivati"
      ],
      "allergens_source": "Dishcovery",
      "image": "",
      "payoff": "",
      "order": 14
    },
    {
      "id": 307900,
      "name": "Messicana",
      "price": 10.5,
      "description": "",
      "ingredients": "Fior di Latte, Pomodoro, Salame Piccante, Pancetta, Fagioli, Peperoncino",
      "allergens": [
        "Glutine",
        "Latte e derivati"
      ],
      "allergens_source": "Dishcovery",
      "image": "",
      "payoff": "",
      "order": 15
    },
    {
      "id": 309810,
      "name": "Marinara",
      "price": 6.0,
      "description": "",
      "ingredients": "Pomodoro, Prezzemolo, Aglio",
      "allergens": [
        "Glutine"
      ],
      "allergens_source": "Dishcovery",
      "image": "",
      "payoff": "",
      "order": 16
    },
    {
      "id": 307901,
      "name": "Misteriosa",
      "price": 12.0,
      "description": "",
      "ingredients": "Fior di Latte, Pomodoro, Salsiccia, Grana Padano, Rucola, Funghi Porcini, Zucchine",
      "allergens": [],
      "allergens_source": "",
      "image": "",
      "payoff": "",
      "order": 17
    },
    {
      "id": 309618,
      "name": "Napoli",
      "price": 8.0,
      "description": "",
      "ingredients": "Fior di Latte, Pomodoro, Acciughe, Origano",
      "allergens": [
        "Glutine",
        "Latte e derivati",
        "Pesce"
      ],
      "allergens_source": "Dishcovery",
      "image": "",
      "payoff": "",
      "order": 18
    },
    {
      "id": 307902,
      "name": "Parmigiana",
      "price": 9.5,
      "description": "",
      "ingredients": "Fior di Latte, Pomodoro, Melanzane, Grana Padano",
      "allergens": [
        "Glutine",
        "Latte e derivati"
      ],
      "allergens_source": "Dishcovery",
      "image": "",
      "payoff": "",
      "order": 19
    },
    {
      "id": 307904,
      "name": "Ricca",
      "price": 12.5,
      "description": "",
      "ingredients": "Fior di Latte, Mozzarella di Bufala, Gorgonzola, Scamorza, Prosciutto Crudo di Parma",
      "allergens": [],
      "allergens_source": "",
      "image": "",
      "payoff": "",
      "order": 20
    },
    {
      "id": 307903,
      "name": "Raffinata",
      "price": 10.0,
      "description": "",
      "ingredients": "Fior di Latte, Pomodoro, Prosciutto Crudo di Parma, Grana Padano, Rucola",
      "allergens": [
        "Glutine",
        "Latte e derivati"
      ],
      "allergens_source": "Dishcovery",
      "image": "",
      "payoff": "",
      "order": 21
    },
    {
      "id": 309620,
      "name": "Romana",
      "price": 8.5,
      "description": "",
      "ingredients": "Fior di Latte, Pomodoro, Acciughe, Capperi",
      "allergens": [
        "Glutine",
        "Latte e derivati"
      ],
      "allergens_source": "Dishcovery",
      "image": "",
      "payoff": "",
      "order": 22
    },
    {
      "id": 307905,
      "name": "Rustica",
      "price": 10.5,
      "description": "",
      "ingredients": "Fior di Latte, Pomodoro, Gorgonzola, Funghi Porcini, Rucola",
      "allergens": [
        "Glutine",
        "Latte e derivati"
      ],
      "allergens_source": "Dishcovery",
      "image": "",
      "payoff": "",
      "order": 23
    },
    {
      "id": 307906,
      "name": "Saporita",
      "price": 10.0,
      "description": "",
      "ingredients": "Fior di Latte, Pomodoro, Salame Piccante, Salsiccia, Wurstel",
      "allergens": [],
      "allergens_source": "",
      "image": "",
      "payoff": "",
      "order": 24
    },
    {
      "id": 307907,
      "name": "Sfiziosa",
      "price": 11.0,
      "description": "",
      "ingredients": "Fior di Latte, Pomodoro, Scamorza, Funghi Porcini, Speck",
      "allergens": [],
      "allergens_source": "",
      "image": "",
      "payoff": "",
      "order": 25
    },
    {
      "id": 307908,
      "name": "Stravagante",
      "price": 10.5,
      "description": "",
      "ingredients": "Fior di Latte, Pomodoro, Scamorza, Pancetta, Radicchio, Aceto Balsamico",
      "allergens": [
        "Glutine",
        "Latte e derivati"
      ],
      "allergens_source": "Dishcovery",
      "image": "",
      "payoff": "",
      "order": 26
    },
    {
      "id": 307909,
      "name": "Tirolese",
      "price": 11.0,
      "description": "",
      "ingredients": "Fior di Latte, Pomodoro, Speck, Funghi Porcini, Gorgonzola",
      "allergens": [],
      "allergens_source": "",
      "image": "",
      "payoff": "",
      "order": 28
    },
    {
      "id": 307910,
      "name": "Trevisana",
      "price": 9.0,
      "description": "",
      "ingredients": "Fior di Latte, Pomodoro, Pancetta, Radicchio",
      "allergens": [
        "Glutine",
        "Latte e derivati"
      ],
      "allergens_source": "Dishcovery",
      "image": "",
      "payoff": "",
      "order": 29
    },
    {
      "id": 307912,
      "name": "Vegetariana",
      "price": 10.0,
      "description": "",
      "ingredients": "Fior di Latte, Pomodoro, Grana Padano, Verdure Fresche",
      "allergens": [
        "Glutine",
        "Latte e derivati"
      ],
      "allergens_source": "Dishcovery",
      "image": "",
      "payoff": "",
      "order": 30
    },
    {
      "id": 307913,
      "name": "Verdure Grigliate",
      "price": 9.0,
      "description": "",
      "ingredients": "Fior di Latte, Pomodoro, Peperoni, Zucchine, Melanzane",
      "allergens": [],
      "allergens_source": "",
      "image": "",
      "payoff": "",
      "order": 31
    },
    {
      "id": 307914,
      "name": "Vigliacca",
      "price": 10.0,
      "description": "",
      "ingredients": "Fior di Latte, Pomodoro, Gorgonzola, Pancetta, Spinaci",
      "allergens": [],
      "allergens_source": "",
      "image": "",
      "payoff": "",
      "order": 32
    },
    {
      "id": 307919,
      "name": "Viking",
      "price": 11.5,
      "description": "",
      "ingredients": "Fior di Latte, Pomodoro, Funghi Porcini, Salsiccia, Cipolla, Olive Taggiasche",
      "allergens": [
        "Glutine",
        "Latte e derivati"
      ],
      "allergens_source": "Dishcovery",
      "image": "",
      "payoff": "",
      "order": 33
    },
    {
      "id": 307916,
      "name": "Vulcano",
      "price": 10.0,
      "description": "",
      "ingredients": "Fior di Latte, Pomodoro, Wurstel, Salame Piccante, Funghi Champignon, Pancetta",
      "allergens": [
        "Glutine",
        "Latte e derivati"
      ],
      "allergens_source": "Dishcovery",
      "image": "",
      "payoff": "",
      "order": 34
    }
  ],
  "secondi": [
    {
      "id": 307686,
      "name": "Galletto a Bassa Temperatura agli Agrumi con Spinaci al Vapore e Riduzione alla Soia",
      "price": 15.0,
      "description": "",
      "ingredients": "Galletto cotto a bassa temperatura con salsa agli agrumi, Erbette fresche al vapore con glassa \talla salsa di soia",
      "allergens": [
        "Soia"
      ],
      "allergens_source": "Dishcovery",
      "image": "assets/menu-images/307686_Galletto_a_Bassa_Temperatura_agli_Agrumi_con_Spinaci_al_Vapore_e_Riduzione_alla_Soia.webp",
      "payoff": "",
      "order": 0
    },
    {
      "id": 307947,
      "name": "Tagliata di Controfiletto di Manzo \"Argentino\", al Sale di Cervia con Patate Rustiche al Forno",
      "price": 22.0,
      "description": "",
      "ingredients": "Controfiletto Di Manzo, Aglio, Rosmarino, Patata",
      "allergens": [],
      "allergens_source": "",
      "image": "assets/menu-images/307947_Tagliata_di_Controfiletto_di_Manzo_Argentino_al_Sale_di_Cervia_con_Patate_Rustiche_al_Forno.webp",
      "payoff": "",
      "order": 1
    },
    {
      "id": 493678,
      "name": "Baccalà Mantecato su Crema di Cipolle al Nero di Seppia e Pesto di Pomodori Secchi",
      "price": 15.0,
      "description": "Un elegante incontro di sapori: morbido baccalà mantecato servito su una vellutata crema di cipolle arricchita dal sapore profondo del nero di seppia. Il piatto è completato da un vibrante pesto di pomodori secchi, che aggiunge un tocco di dolcezza e acidità. Una combinazione raffinata di mare e terra, perfetta per chi desidera un'esperienza gastronomica sofisticata e ricca di contrasti.",
      "ingredients": "",
      "allergens": [
        "Pesce"
      ],
      "allergens_source": "Dishcovery",
      "image": "assets/menu-images/493678_Baccal_Mantecato_su_Crema_di_Cipolle_al_Nero_di_Seppia_e_Pesto_di_Pomodori_Secchi.webp",
      "payoff": "",
      "order": 2
    },
    {
      "id": 307949,
      "name": "Filetto di Manzo \"Angus\" ai Porcini, preparato a Bassa Temperatura",
      "price": 24.0,
      "description": "Un pregiato filetto di manzo Angus, cotto lentamente a bassa temperatura per preservarne tutta la tenerezza e i sapori naturali. Accompagnato da un ricco e aromatico condimento di funghi porcini, che esalta la succosità della carne, questo piatto offre un’esperienza culinaria intensa e raffinata. Una scelta perfetta per gli amanti della carne, dove la delicatezza della cottura sous-vide incontra la robustezza dei funghi porcini, in un’armonia di sapori autentici e avvolgenti.",
      "ingredients": "Filetto Di Manzo, Aglio, Porcini",
      "allergens": [],
      "allergens_source": "",
      "image": "",
      "payoff": "",
      "order": 3
    },
    {
      "id": 307950,
      "name": "Filetto di Manzo \"Angus\" al Pepe Rosa",
      "price": 19.0,
      "description": "",
      "ingredients": "Filetto Di Manzo, Cognac, Pepe Verde",
      "allergens": [],
      "allergens_source": "",
      "image": "",
      "payoff": "",
      "order": 4
    },
    {
      "id": 307953,
      "name": "Polpo Cotto a Bassa Temperatura su Crema di Patate e Gocce di Prezzemolo",
      "price": 15.0,
      "description": "Un polpo selezionato dalle coste del Marocco, cotto a bassa temperatura per ottenere una consistenza incredibilmente tenera e succulenta. Servito su una delicata crema di patate vellutata, impreziosita da fresche gocce di prezzemolo che aggiungono una nota erbacea e vivace. Un piatto elegante e bilanciato, dove la morbidezza del polpo si sposa perfettamente con la cremosità delle patate e la freschezza del prezzemolo, per un’esperienza gustativa raffinata e avvolgente.",
      "ingredients": "Polpo, Scalogno, Patate, Prezzemolo, Sedano, Carote, Cipolla, Olio",
      "allergens": [
        "Pesce",
        "Sedano"
      ],
      "allergens_source": "Dishcovery",
      "image": "assets/menu-images/307953_Polpo_Cotto_a_Bassa_Temperatura_su_Crema_di_Patate_e_Gocce_di_Prezzemolo.webp",
      "payoff": "",
      "order": 5
    },
    {
      "id": 307951,
      "name": "Fritto di Mare e Verdure Croccanti",
      "price": 16.0,
      "description": "Sapori marini e freschezza vegetale si incontrano in questa deliziosa frittura mista. Mazzancolle succulente, anelli di calamari teneri, alici aromatiche, melanzane croccanti e zucchine saporite. Un piatto irresistibilmente croccante e gustoso.",
      "ingredients": "Mazzancolle, Calamari, Melanzane, Zucchine",
      "allergens": [
        "Crostacei",
        "Glutine",
        "Molluschi",
        "Pesce"
      ],
      "allergens_source": "Dishcovery",
      "image": "",
      "payoff": "",
      "order": 6
    },
    {
      "id": 380901,
      "name": "Grigliata di Carne con Patate al Forno",
      "price": 18.0,
      "description": "Salsiccia, Petto di Pollo, Pancetta, Lonza e Patate al Forno",
      "ingredients": "Salsiccia, Petto di Pollo, Lonza di Maiale, Pancetta, Patate al Forno",
      "allergens": [],
      "allergens_source": "",
      "image": "",
      "payoff": "",
      "order": 7
    },
    {
      "id": 304044,
      "name": "Cotoletta alla milanese con patate fritte",
      "price": 15.0,
      "description": "",
      "ingredients": "",
      "allergens": [],
      "allergens_source": "",
      "image": "",
      "payoff": "",
      "order": 8
    }
  ],
  "contorni": [
    {
      "id": 318869,
      "name": "Patate Fritte",
      "price": 4.5,
      "description": "",
      "ingredients": "",
      "allergens": [],
      "allergens_source": "",
      "image": "",
      "payoff": "",
      "order": 0
    },
    {
      "id": 318870,
      "name": "Insalata Mista",
      "price": 5.0,
      "description": "",
      "ingredients": "",
      "allergens": [],
      "allergens_source": "",
      "image": "",
      "payoff": "",
      "order": 1
    },
    {
      "id": 520065,
      "name": "Melanzane e Zucchine Fritte",
      "price": 5.0,
      "description": "",
      "ingredients": "",
      "allergens": [],
      "allergens_source": "",
      "image": "",
      "payoff": "",
      "order": 43
    },
    {
      "id": 520066,
      "name": "Anelli di Cipolla Fritte",
      "price": 4.5,
      "description": "",
      "ingredients": "",
      "allergens": [],
      "allergens_source": "",
      "image": "",
      "payoff": "",
      "order": 44
    },
    {
      "id": 668297,
      "name": "Verdure Grigliate",
      "price": 5.0,
      "description": "Melanzane e Zucchine",
      "ingredients": "Melanzane, Zucchine",
      "allergens": [],
      "allergens_source": "",
      "image": "",
      "payoff": "",
      "order": 46
    }
  ],
  "dessert": [
    {
      "id": 304123,
      "name": "Mascarpone",
      "price": 5.0,
      "description": "",
      "ingredients": "Mascarpone, Uova, zucchero",
      "allergens": [
        "Latte e derivati",
        "Uova e derivati"
      ],
      "allergens_source": "Dishcovery",
      "image": "",
      "payoff": "",
      "order": 0
    },
    {
      "id": 304125,
      "name": "Tiramisù",
      "price": 5.0,
      "description": "",
      "ingredients": "",
      "allergens": [
        "Latte e derivati",
        "Uova e derivati"
      ],
      "allergens_source": "Dishcovery",
      "image": "",
      "payoff": "",
      "order": 2
    },
    {
      "id": 307970,
      "name": "Crema Catalana",
      "price": 5.0,
      "description": "",
      "ingredients": "",
      "allergens": [],
      "allergens_source": "",
      "image": "",
      "payoff": "",
      "order": 3
    },
    {
      "id": 661435,
      "name": "Tortino di Cioccolato con Cuore Fondente",
      "price": 5.0,
      "description": "",
      "ingredients": "",
      "allergens": [],
      "allergens_source": "",
      "image": "",
      "payoff": "",
      "order": 5
    }
  ],
  "birre": [
    {
      "id": "local-birre-1",
      "name": "Moretti Filtrata a Freddo",
      "price": 5.0,
      "description": "Formato: 55 cl • 4,3% Vol",
      "ingredients": "",
      "allergens": [],
      "allergens_source": "",
      "image": "",
      "payoff": "",
      "order": 0
    },
    {
      "id": "local-birre-2",
      "name": "Ichnusa Non Filtrata",
      "price": 5.0,
      "description": "Formato: 50 cl",
      "ingredients": "",
      "allergens": [],
      "allergens_source": "",
      "image": "",
      "payoff": "",
      "order": 1
    },
    {
      "id": "local-birre-3",
      "name": "Messina – Bionda Italiana",
      "price": 5.0,
      "description": "Formato: 50 cl",
      "ingredients": "",
      "allergens": [],
      "allergens_source": "",
      "image": "",
      "payoff": "",
      "order": 2
    },
    {
      "id": "local-birre-4",
      "name": "Beck's – Tedesca Bionda",
      "price": 4.0,
      "description": "Formato: 33 cl",
      "ingredients": "",
      "allergens": [],
      "allergens_source": "",
      "image": "",
      "payoff": "",
      "order": 3
    },
    {
      "id": "local-birre-5",
      "name": "Ceres",
      "price": 4.5,
      "description": "Formato: 33 cl • 7,7% Vol",
      "ingredients": "",
      "allergens": [],
      "allergens_source": "",
      "image": "",
      "payoff": "",
      "order": 4
    },
    {
      "id": "local-birre-6",
      "name": "Erdinger",
      "price": 5.0,
      "description": "Formato: 50 cl • 5,3% Vol",
      "ingredients": "",
      "allergens": [],
      "allergens_source": "",
      "image": "",
      "payoff": "",
      "order": 5
    },
    {
      "id": "local-birre-7",
      "name": "Paulaner",
      "price": 5.0,
      "description": "Formato: 50 cl",
      "ingredients": "",
      "allergens": [],
      "allergens_source": "",
      "image": "",
      "payoff": "",
      "order": 6
    },
    {
      "id": "local-birre-8",
      "name": "Leffe \"Blonde\"",
      "price": 10.0,
      "description": "Formato: 75 cl • 6,6% Vol",
      "ingredients": "",
      "allergens": [],
      "allergens_source": "",
      "image": "",
      "payoff": "",
      "order": 7
    },
    {
      "id": "local-birre-9",
      "name": "Leffe \"Rouge\"",
      "price": 10.0,
      "description": "Formato: 75 cl • 6,6% Vol",
      "ingredients": "",
      "allergens": [],
      "allergens_source": "",
      "image": "",
      "payoff": "",
      "order": 8
    }
  ],
  "vini_rossi": [
    {
      "id": "local-vini_rossi-1",
      "name": "Sangiovese di Romagna \"Notturno\" DOC",
      "price": 22.0,
      "description": "Formato: 75 cl • Tenuta Drei Donà",
      "ingredients": "",
      "allergens": [],
      "allergens_source": "",
      "image": "",
      "payoff": "",
      "order": 0
    },
    {
      "id": "local-vini_rossi-2",
      "name": "Sangiovese Superiore \"Prugneto\" DOC",
      "price": 20.0,
      "description": "Formato: 75 cl • Poderi Dal Nespoli",
      "ingredients": "",
      "allergens": [],
      "allergens_source": "",
      "image": "",
      "payoff": "",
      "order": 1
    },
    {
      "id": "local-vini_rossi-3",
      "name": "Sangiovese Superiore \"Ceregio Rosso\" DOC",
      "price": 15.0,
      "description": "Formato: 75 cl • Fattoria Zerbina",
      "ingredients": "",
      "allergens": [],
      "allergens_source": "",
      "image": "",
      "payoff": "",
      "order": 2
    },
    {
      "id": "local-vini_rossi-4",
      "name": "Sangiovese Superiore \"Ceregio Rosso\" DOC – Mezza bottiglia",
      "price": 10.0,
      "description": "Formato: 37,5 cl • Fattoria Zerbina",
      "ingredients": "",
      "allergens": [],
      "allergens_source": "",
      "image": "",
      "payoff": "",
      "order": 3
    },
    {
      "id": "local-vini_rossi-5",
      "name": "Valpolicella Superiore \"Ripasso\" DOC",
      "price": 18.0,
      "description": "Formato: 75 cl • Casa Sartori",
      "ingredients": "",
      "allergens": [],
      "allergens_source": "",
      "image": "",
      "payoff": "",
      "order": 4
    }
  ],
  "vini_bianchi": [
    {
      "id": "local-vini_bianchi-1",
      "name": "Gewürztraminer Trentino DOC",
      "price": 17.0,
      "description": "Formato: 75 cl • Mezzacorona",
      "ingredients": "",
      "allergens": [],
      "allergens_source": "",
      "image": "",
      "payoff": "",
      "order": 0
    },
    {
      "id": "local-vini_bianchi-2",
      "name": "Falanghina del Sannio DOC",
      "price": 20.0,
      "description": "Formato: 75 cl • Feudi di San Gregorio",
      "ingredients": "",
      "allergens": [],
      "allergens_source": "",
      "image": "",
      "payoff": "",
      "order": 1
    },
    {
      "id": "local-vini_bianchi-3",
      "name": "Ribolla Gialla delle Venezie IGT",
      "price": 17.0,
      "description": "Formato: 75 cl • Villa Folini",
      "ingredients": "",
      "allergens": [],
      "allergens_source": "",
      "image": "",
      "payoff": "",
      "order": 2
    },
    {
      "id": "local-vini_bianchi-4",
      "name": "Greco di Tufo DOC",
      "price": 20.0,
      "description": "Formato: 75 cl • Feudi di San Gregorio",
      "ingredients": "",
      "allergens": [],
      "allergens_source": "",
      "image": "",
      "payoff": "",
      "order": 3
    },
    {
      "id": "local-vini_bianchi-5",
      "name": "Prosecco Superiore Valdobbiadene DOC",
      "price": 15.0,
      "description": "Formato: 75 cl • Ca Val",
      "ingredients": "",
      "allergens": [],
      "allergens_source": "",
      "image": "",
      "payoff": "",
      "order": 4
    }
  ],
  "bevande": [
    {
      "id": "local-bevande-1",
      "name": "Acqua naturale Pejo in vetro",
      "price": 3.0,
      "description": "Formato: 75 cl",
      "ingredients": "",
      "allergens": [],
      "allergens_source": "",
      "image": "",
      "payoff": "",
      "order": 0
    },
    {
      "id": "local-bevande-2",
      "name": "Acqua gassata Pejo in vetro",
      "price": 3.0,
      "description": "Formato: 75 cl",
      "ingredients": "",
      "allergens": [],
      "allergens_source": "",
      "image": "",
      "payoff": "",
      "order": 1
    },
    {
      "id": "local-bevande-3",
      "name": "Coca-Cola in vetro 1 L",
      "price": 8.0,
      "description": "Formato: 1 L",
      "ingredients": "",
      "allergens": [],
      "allergens_source": "",
      "image": "",
      "payoff": "",
      "order": 2
    },
    {
      "id": "local-bevande-4",
      "name": "Coca-Cola in vetro 33 cl",
      "price": 3.5,
      "description": "Formato: 33 cl",
      "ingredients": "",
      "allergens": [],
      "allergens_source": "",
      "image": "",
      "payoff": "",
      "order": 3
    },
    {
      "id": "local-bevande-5",
      "name": "Fanta in PET",
      "price": 3.5,
      "description": "Formato: 45 cl",
      "ingredients": "",
      "allergens": [],
      "allergens_source": "",
      "image": "",
      "payoff": "",
      "order": 4
    }
  ]
};
